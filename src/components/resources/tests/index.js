...
export default () => {
  ...
  describe('list route', () => {
    ...
    it('succeeds with basic options', () => {
      ...
      expect(list.handler.beforeProcessor.length).to.equal(1)
      expect(list.handler.afterProcessor).to.be.empty
      ...
      expect(list.handler.afterCommit).to.be.empty
      expect(list.handler.beforeRollback).to.be.empty
    })
